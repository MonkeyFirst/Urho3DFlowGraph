...
public:
	...
	SharedPtr<FlowGraph> flowGraphLight;


	Game(Context* context) : Application(context)
	{
		CustomFlowNodeLamp::RegisterObject(context);
	}
	...
	void Start()
	{
		context_->RegisterSubsystem(new FlowGraphSystem(context_));
		...
		CreateFlowGraph();
		CreateFlowGraphWithCustomFlowNode();
	}
	...
	void CreateFlowGraph()
	{
		...
		GetSubsystem<FlowGraphSystem>()->PushUpdateFlowGraph(flowGraphExample_);
		GetSubsystem<FlowGraphSystem>()->PushUpdateFlowGraph(flowGraphTimerWithCubeCreate_);
	}

	void CreateFlowGraphWithCustomFlowNode() 
	{
		Node* lightNode = scene_->CreateChild("LightNode");
		lightNode->SetPosition(Vector3(0,2,0));
		Light* light_= lightNode->CreateComponent<Light>();
		light_->SetColor(Color(1, 0, 0));
		light_->SetBrightness(3.0f);
		light_->SetLightType(LightType::LIGHT_POINT);
		CustomFlowNodeLamp* customFlowNode = lightNode->CreateComponent<CustomFlowNodeLamp>();
		FlowNode* flownode_ = customFlowNode->GetFlowNode();

		// Создаем граф 3.
		SharedPtr<TimerFlowNode> TimerNode = SharedPtr<TimerFlowNode>(new TimerFlowNode(context_));
		TimerNode->inputs_[TimerFlowNode::Period]->data_ = Variant(1.0f); // sec
		TimerNode->inputs_[TimerFlowNode::Active]->data_ = true; // sec


		SharedPtr<ToggleSwitchFlowNode> ToggleNode = SharedPtr<ToggleSwitchFlowNode>(new ToggleSwitchFlowNode(context_));
	
		flowGraphLight = new FlowGraph(context_);
		flowGraphLight->SetName("LightFlowGraph");

		flowGraphLight->nodes_.Push(TimerNode);
		flowGraphLight->nodes_.Push(ToggleNode);
		flowGraphLight->nodes_.Push(customFlowNode->GetFlowNode());

		flowGraphLight->Connect(TimerNode->outputs_[TimerFlowNode::TimerEvent],
			ToggleNode->inputs_[ToggleSwitchFlowNode::Toggle]);

		flowGraphLight->Connect(ToggleNode->outputs_[ToggleSwitchFlowNode::TurnOn],
			flownode_->inputs_[CustomFlowNodeLamp::Enable]);

		GetSubsystem<FlowGraphSystem>()->PushUpdateFlowGraph(flowGraphLight);
